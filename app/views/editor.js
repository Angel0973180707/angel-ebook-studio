...
export function mountEditor(root, { store, bookId, toast }){
  ...
  if(!book){
    root.innerHTML = `<section class="card editor"><div class="panelHead"><div>找不到這本書</div></div><div class="panelBody"><button class="btn" onclick="location.hash=''">回到書庫</button></div></section>`;
    return;
  }
  ...
  <section class="card editor" data-book="${book.id}">
    <div class="panelHead">
      ...
      <div class="right">
        <span class="pill ${book.status==='published'?'ok':''}">${book.status==='published'?'完稿':'草稿'}</span>
      </div>
    </div>
  ...
  function switchTab(tab){
    ...
    root.querySelectorAll('.tabBody').forEach(b=> b.hidden = b.dataset.tab!==tab);
  }
  ...
  root.querySelectorAll('textarea[data-key]').forEach(el => {
    el.addEventListener('input', e => {
      ...
      store.updateBook(book.id, { pages: { ...cur.pages, [key]: el.value } });
    });
  });
  ...
  root.querySelectorAll('[data-cmd]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      let clip = '';
      if(cmd==='copy-chapter'){
        clip = templates.chapter();
      }else if(cmd==='copy-deai'){
        clip = templates.deai();
      }else if(cmd==='copy-exercise'){
        clip = templates.exercise();
      }
      if(clip){
        navigator.clipboard.writeText(clip).then(()=> toast('已複製到剪貼簿，可貼到 ChatGPT'));
      }
    });
  });
}
...
function area(key, value, hint){
  return `
    <textarea class="textarea" data-key="\${key}" placeholder="\${hint}">\${escapeHTML(value||'')}</textarea>
